...
//return a vector of strings(board) for the next move
...
//returns the result of the board if ended, returns 2 if not ended
int result(string board){
  //result is based on computer (1 for computer win)
  //check for horizontal match (in a row)
  for (int i=0;i<9;i+=3){
  ...
  //check for vertical match (in a column)
  for (int i=0;i<9;i++){
    //check if there is no pieces on the position
    if (board[i]==' '){
      continue;
...
//if player is X, it is the maximizing player
